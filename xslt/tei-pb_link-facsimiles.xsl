...
<xsl:stylesheet exclude-result-prefixes="xs xd html" version="2.0"
    ...
    <xsl:template match="tei:revisionDesc" priority="100">
        ...
        <xsl:copy>
            ...
            <xsl:element name="tei:change">
                ...
                <xsl:text>Linked all </xsl:text>
                <tei:tag>pb ed="print"</tei:tag>
                <xsl:text> to the corresponding facsimile</xsl:text>
            </xsl:element>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="tei:pb[@ed = 'print']">
        ...
        <xsl:variable name="v_id-facs"
            select="concat('#', $vFacs/descendant::tei:surface[$vPosPb]/@xml:id)"/>
        <xsl:copy>
            <!-- add documentation of change -->
            <xsl:apply-templates select="@*"/>
            <xsl:if test="not(@facs) or not(@facs = $v_id-facs)">
                <xsl:attribute name="facs">
                    <xsl:value-of select="$v_id-facs"/>
                </xsl:attribute>
                <xsl:choose>
