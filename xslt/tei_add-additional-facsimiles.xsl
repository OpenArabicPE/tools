...
<xsl:stylesheet exclude-result-prefixes="xs xd html" version="3.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xd="http://www.pnp-software.com/XSLTdoc"
    ...
    <xsl:param as="xs:integer" name="p_image-setoff_hathi" select="0"/>
    <!-- set-off between EAP image number and the printed edition; default is 0 -->
    <xsl:param as="xs:integer" name="p_image-setoff_eap" select="5"/>
    <!-- set-off between local image number and the printed edition; default is 0 -->
    <xsl:param as="xs:integer" name="p_image-setoff_local" select="0"/>
    ...
    <xsl:variable as="xs:integer" name="v_publication_eap" select="8"/>
    <xsl:param as="xs:integer" name="p_volume-setoff_eap" select="-1"/>
    <!-- IIIF -->
    ...
    <xsl:variable as="xs:integer" name="v_page-start">
        <xsl:choose>
    ...
    <xsl:variable as="xs:integer" name="v_pages">
        <xsl:choose>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$p_file-eap = true()">
                        ...
                        <xsl:attribute name="url"
                            select="concat($v_eap-iiif-scheme, $v_eap-iiif-server, $v_eap-iiif-prefix, '/', replace($v_eap-id, '/', '_'), '/', $v_page + $p_image-setoff_eap, '.jp2', $v_iiif-settings-max)"/>
                    ...
                    <xsl:when test="$p_file-hathi = true()">
                        <xsl:attribute name="url" select="concat($v_url-hathi, $v_page + $p_image-setoff_hathi)"/>
                        <xsl:attribute name="mimeType" select="'image/jpeg'"/>
                    </xsl:when>
                </xsl:choose>
            </xsl:element>
    ...
    <xsl:template match="tei:revisionDesc">
        <xsl:copy>
            ...
            <xsl:element name="tei:change">
                ...
                <tei:gi>graphic</tei:gi>
                <xsl:text> for all pages linking to </xsl:text>
        ...
        <xsl:element name="tei:surface">
            ...
            <xsl:if test="$p_file-eap = true()">
                <xsl:element name="tei:graphic">
                    ...
                    <!-- new url pointing to IIIF manifest -->
                    <xsl:attribute name="url"
                        select="concat($v_eap-iiif-scheme, $v_eap-iiif-server, $v_eap-iiif-prefix, '/EAP119_1_', $v_publication_eap, '_', $v_volume + $p_volume-setoff_eap, '/', $p_page-start + $p_image-setoff_eap, '.jp2')"/>
                    <xsl:attribute name="mimeType" select="'image/jpeg'"/>
                    <xsl:attribute name="type" select="'iiif'"/>
                </xsl:element>
            </xsl:if>
