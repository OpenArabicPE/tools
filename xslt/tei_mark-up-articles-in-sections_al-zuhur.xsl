...
<xsl:stylesheet exclude-result-prefixes="xs" version="3.0" xmlns:tei="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:param name="p_section-title" select="'في رياض الشعر'"/>
    ...
    <xsl:template
        match="tei:div[@type = 'section'][not(ancestor::tei:div[@type = ('article', 'item')])][tei:head = $p_section-title]">
            ...
            <xsl:choose>
               ...
               <!-- <xsl:when test="tei:p[tei:hi[@style = 'color:red;']]">
                ...
                <xsl:when
                    ...
                    />
                    <!-- for sections with only a single article -->
                    <!--<xsl:element name="tei:div">
            <xsl:attribute name="type" select="'item'"/>
            <xsl:attribute name="subtype" select="'article'"/>
            <xsl:attribute name="change" select="concat('#', $p_id-change)"/>
                        <xsl:attribute name="xml:lang" select="@xml:lang"/>
                        <xsl:apply-templates select="node()[not(self::tei:head)]" mode="m_convert-p-to-head"/>
                        <!-\-<xsl:apply-templates select="node()[not(self::tei:p[1])]"/>-\->
                    </xsl:element>-->
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates/>
    ...
    <xsl:template match="node()" mode="m_convert-p-to-head">
        <xsl:choose>
            <xsl:when test=". = parent::tei:div/tei:p[1]">
